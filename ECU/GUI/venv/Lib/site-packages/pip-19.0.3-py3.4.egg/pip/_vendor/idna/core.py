...
def valid_contextj(label, pos):
    ...
    if cp_value == 0x200C:
        ...
        if pos > 0:
            if _combining_class(ord(label[pos -
...
def valid_contexto(label, pos, exception=False):
    ...
    if cp_value == 0x00B7:
        ...
        for cp in label:
            if cp == u"\u30fb":
                continue
            if (_is_script(cp, "Hiragana") or _is_script(cp, "Katakana")
...
def check_label(label):
    ...
    for (pos, cp) in enumerate(label):
        ...
        if intranges_contain(cp_value, idnadata.codepoint_classes["PVALID"]):
            continue
        elif intranges_contain(cp_value,
            ...
            try:
                ...
                raise IDNAError(
                    "Unknown codepoint adjacent to joiner {0} at position {1} in {2}"
...
def uts46_remap(domain, std3_rules=True, transitional=False):
    ...
    try:
        for pos, char in enumerate(domain):
            code_point = ord(char)
            uts46row = uts46data[code_point if code_point < 256 else bisect.
                                 bisect_left(uts46data, (code_point, "Z")) - 1]
            status = uts46row[1]
