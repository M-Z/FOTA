    ...
    try:
        ...
        try:
            while read_count < requested_length:
                ...
                buffer = (ctypes.c_char * remaining).from_address(data_buffer +
                                                                  read_count)
...
class WrappedSocket(object):
    ...
    def _set_ciphers(self):
        ...
        result = Security.SSLSetEnabledCiphers(self.context, ciphers,
                                               len(CIPHER_SUITES))
    ...
    def _custom_validate(self, verify, trust_bundle):
        ...
        try:
            ...
            result = Security.SSLCopyPeerTrust(self.context,
                                               ctypes.byref(trust))
            _assert_no_error(result)
            if not trust:
            ...
            result = Security.SecTrustEvaluate(trust,
        ...
        if trust_result.value not in successes:
            raise ssl.SSLError("certificate verify failed, error code: %d" %
                               trust_result.value)
    ...
    def recv_into(self, buffer, nbytes=None):
        ...
        with self._raise_on_error():
            result = Security.SSLRead(self.context, buffer, nbytes,
    ...
    def send(self, data):
        ...
        with self._raise_on_error():
            result = Security.SSLWrite(self.context, data, len(data),
                                       ctypes.byref(processed_bytes))
