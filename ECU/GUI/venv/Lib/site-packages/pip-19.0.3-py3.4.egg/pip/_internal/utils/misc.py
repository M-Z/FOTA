...
def get_installed_distributions(
    ...
    return [
        d for d in pkg_resources.working_set  # type: ignore
        if local_test(d) and d.key not in skip and editable_test(d)
        and editables_only_test(d) and user_test(d)
    ]
...
def call_subprocess(
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == "raise":
            ...
            ) > std_logging.DEBUG and not show_stdout:
                logger.info(
...
def split_auth_from_netloc(netloc):
    ...
    user_pass = tuple(None if x is None else urllib_unquote(x)
                      for x in user_pass)
