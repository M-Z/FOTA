...
void EndofConversionReset(void) {
  ...
  while (((ADC1_SR & 0x00000002) >> 1) == 1)
    ; // waiting for EOC reset flag
}
...
void SequenceRegistersInit(void) {
  ...
  ADC1_SQR3 |= ((SQ6 << 25) | (SQ5 << 20) | (SQ4 << 15) | (SQ3 << 10) |
                (SQ2 << 5) | (SQ1));
  ADC1_SQR2 |= ((SQ12 << 25) | (SQ11 << 20) | (SQ10 << 15) | (SQ9 << 10) |
                (SQ8 << 5) | (SQ7));
...
void ADC_Init() {
  ...
  // ADC1_CR1 |= EOCINTERRUPTENABLE;				//EOC interrupt
  // enable
}
...
// void EndofConversionReset (void)
//{
//	ADC1_SR &= ~(0x00000002);			//reset EOC after reading
//from DR 	while (((ADC1_SR & 0x00000002) >>1)	== 1);		//waiting for
//EOC reset flag
//}
//
// void SequenceRegistersInit(void)
...
// SequenceRegistersInit();			//to select the used channels
//
// ADC1_CR2|= SOCTRIGGER;				//choose SWSTART as start of
// conversion trigger
//
//
//
...
//
//	 //EndofConversionReset ();
////	WaitingStartConversion ();		//waiting for start conversion
///flag
//
//	//WaitingEndofConversion ();	//waiting for EOC
