...
void BOOTLOADER_vidStart(u8 u8ProgrammingBank, u8 u8OptionDataBytes) {
  ...
  while (1) {
        ...
        if (CAN_RxRdy) {
          ...
          if (CAN_RxMsg[u8RxCount].u8ActiveFlag == 1) {
            ...
            case CANHANDLER_u8HEXFILEID:
              for (u8Counter = 0; u8Counter < CAN_RxMsg[u8RxCount].len;
                   u8Counter++) {
                HexArrayLine[u16LineCounter] =
                    CAN_RxMsg[u8RxCount].data[u8Counter];
                u16LineCounter++;
              }
              CANHANDLER_vidSend(CANHANDLER_u8NEXTMSGREQUEST, CAN_u8REMOTEFRAME,
                                 (void *)0, 0);
            ...
            case CANHANDLER_u8ECUDTCs:
              CANHANDLER_vidSend(CANHANDLER_u8ECUDTCs, CAN_u8DATAFRAME, au8DTCs,
                                 1);
              break;
            }
    ...
    case BOOTLOADER_u8BANK0:
      ...
      u8OptionDataBytes |=
          ((BOOTLOADER_u8VALIDBANK0 << BOOTLOADER_u8VALIDBANK0) |
           (BOOTLOADER_u8BANK0 << BOOTLOADER_u8USEDBANK) |
           (BOOTLOADER_u8OPTIONBYTESVALID << BOOTLOADER_u8VALIDOPTIONBYTES));
      FLASH_vidWriteOptionByteData(FLASH_u8OPTDATA0, u8OptionDataBytes);
    ...
    case BOOTLOADER_u8BANK1:
      ...
      CLR_BIT(u8OptionDataBytes, BOOTLOADER_u8USEDBANK);
      u8OptionDataBytes |=
          ((BOOTLOADER_u8VALIDBANK1 << BOOTLOADER_u8VALIDBANK1) |
           (BOOTLOADER_u8BANK1 << BOOTLOADER_u8USEDBANK) |
           (BOOTLOADER_u8OPTIONBYTESVALID << BOOTLOADER_u8VALIDOPTIONBYTES));
      FLASH_vidWriteOptionByteData(FLASH_u8OPTDATA0, u8OptionDataBytes);
