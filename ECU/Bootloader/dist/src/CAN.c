...
//#include "STM32_Reg.h"                            // STM32 register and bit
//Definitions #include "STM32_Init.h"                           // STM32
//Initialization #include"stm32f10x_rcc.h" #include"stm32f10x_can.h"
...
void CAN_setup(void) {
  ...
  CAN1->MCR = (CAN_MCR_INRQ); // init mode, enable auto. retransmission // Note:
                              // only FIFO 0, transmit mailbox 0 used
  CAN1->MCR |= (1 << CAN_MCR_TXFP); // Transmit priority by request order
  CAN1->IER = (CAN_IER_FMPIE0 | CAN_IER_TMEIE); // FIFO 0 msg pending, Transmit
                                                // mbx empty (enable interrupts)

  /* Note: this calculations fit for PCLK1 = 36MHz */
  brp = (brp / 18) / 500000; // baudrate is set to 500k bit/s

  /* set BTR register so that sample point is at about 72% bit time from bit
   * start */
  /* TSEG1 = 12, TSEG2 = 5, SJW = 4 => 1 CAN bit = 18 TQ, sample at 72%    */
  CAN1->BTR &= ~(((0x03) << 24) | ((0x07) << 20) | ((0x0F) << 16) | (0x1FF));
  CAN1->BTR |= ((((4 - 1) & 0x03) << 24) | (((5 - 1) & 0x07) << 20) |
                (((12 - 1) & 0x0F) << 16) | ((brp - 1) & 0x1FF));
}
...
void CAN_waitReady(void) {
  ...
  while ((CAN1->TSR & CAN_TSR_TME1) == 0)
    ; // Transmit mailbox 1 is empty
  while ((CAN1->TSR & CAN_TSR_TME2) == 0)
    ; // Transmit mailbox 2 is empty
...
void CAN_wrMsg(CAN_msg *msg, u8 u8MailBox) {
  ...
  if (msg->type == DATA_FRAME) { // DATA FRAME
    ...
    CAN1->sTxMailBox[u8MailBox].TDLR =
        (((u32)msg->data[3] << 24) | ((u32)msg->data[2] << 16) |
         ((u32)msg->data[1] << 8) | ((u32)msg->data[0]));
    CAN1->sTxMailBox[u8MailBox].TDHR =
        (((u32)msg->data[7] << 24) | ((u32)msg->data[6] << 16) |
         ((u32)msg->data[5] << 8) | ((u32)msg->data[4]));
  } else { // REMOTE FRAME
...
//
//	CAN1->IER |= CAN_IER_TMEIE;                      // enable  TME
//interrupt 	CAN1->sTxMailBox[1].TIR |=  CAN_TIxR_TXRQ;       // transmit message
//}
//
...
//	CAN1->IER |= CAN_IER_TMEIE;                      // enable  TME
//interrupt 	CAN1->sTxMailBox[2].TIR |=  CAN_TIxR_TXRQ;       // transmit message
//}
...
void CAN_vidReleaseMessage(void) { CAN1->RF0R |= CAN_RF0R_RFOM0; }
...
void USB_LP_CAN1_RX0_IRQHandler(void) {
  ...
  if (CAN1->RF0R & CAN__Msg_Pending) { // message pending ?
