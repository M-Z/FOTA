...
const uint8_t Expected_HMAC[HMAC_LENGTH] =
    "5a68cde2e52250392dae40fc73276197411bf61b4e527118d60a157a0c24b9a0";

/* Private function prototypes -----------------------------------------------*/
int32_t STM32_SHA256_HMAC_Compute(uint8_t *InputMessage,
                                  uint32_t InputMessageLength,
                                  uint8_t *HMAC_key, uint32_t HMAC_keyLength,
...
/* 				use case: Server sends all data in string format so,
 * it convert the HMAC        */
/* 				Key in string format to its Hex decimal format
 */
/* Input      : u8 pBuffer[] is the input string which will be converted */
/* 				u8* hex_ptr2 pointer to an Array of the output
 * converted hex decimal values	    */
/*				int size is the size of the input string in bytes
 */
/* 				int * keySize pointer to the variable to put the
 * coverted hex values size in it */
...
/*********************************************************************************************************/
/* Description: It Generate HMAC and make the comparison between the generated
 * HMAC and the expected one */
/* Input      : const u8 * InputMessage is the input data to check its HMAC
 * (String Format)				 */
...
/* 				u8 * expectedHMAC is pointer to the expected HMAC
 * (String format)                      */
/* Output     : TestStatus, PASSED if the Expected HMAC and the Generated HMAC
 * are identical             */
/* 				Failed if the Expected HMAC and the Generated HMAC
 * are not identical                     */
/* 					   OR  HMAC computing failed */
/* Scope      : Public */ /*********************************************************************************************************/

/*********************************************************************************************************/
TestStatus HMAC_SHA256_Handler(const u8 *InputMessage, u32 InputLength,
                               const u8 *HMAC_key, u32 KeyLength,
                               const u8 *expectedHMAC);

int main(void) {
  TestStatus HMAC_check_Status;
  convertToHex(HMAC_Key_Str, HMAC_Key, sizeof(HMAC_Key_Str), &KeyLengthtemp);
  HMAC_check_Status =
      HMAC_SHA256_Handler(InputData, 5, HMAC_Key, KeyLengthtemp, Expected_HMAC);
 ...
 * @param  MessageDigest: pointer to output parameter that will handle message
 * digest
...
int32_t STM32_SHA256_HMAC_Compute(uint8_t *InputMessage,
  ...
  if (error_status == HASH_SUCCESS) {
    ...
    error_status =
        HMAC_SHA256_Append(&HMAC_SHA256ctx, InputMessage, InputMessageLength);

    if (error_status == HASH_SUCCESS) {
      /* retrieve */
      error_status = HMAC_SHA256_Finish(&HMAC_SHA256ctx, MessageDigest,
                                        MessageDigestLength);
    }
  }
...
TestStatus HMAC_SHA256_Handler(const u8 *InputMessage, u32 InputLength,
  ...
  Crypto_DeInit();
  status =
      STM32_SHA256_HMAC_Compute((uint8_t *)InputMessage, InputLength, HMAC_Key,
                                KeyLength, (uint8_t *)MAC, &MACLength);
