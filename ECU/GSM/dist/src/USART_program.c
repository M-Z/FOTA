...
Error_Status USART_enumInit(u8 Channel) {
  ...
  if (Channel >= CHANNEL_MIN && Channel <= CHANNEL_MAX) {
    ...
    u8 Fraction =
        freq / (16.0 * BAUDRATE) * 100.0 - Mantissa * 100; // Fractional part
 ...
 * 					Can be USART_CHANNEL_x where x is from 1 to
 * 5
 * @param  void (*x)(u16): The function that takes 16 bits argument and returns
 * void
 * @retval Error_Status
 */
...
static void USART_voidSend(volatile UART_REG *selectedChannel, u16 Char) {
  while (!GET_BIT(selectedChannel->SR, 7))
    ; // TXE: The next data can be written in the USART_DR register without
      // overwriting the previous data.
  selectedChannel->DR = Char;
  while (!GET_BIT(selectedChannel->SR, 6))
    ; // TC: Wait for transfer complete
}
...
void USART_voidSendATCommand(u8 Channel, const char *Message) {
  ...
  while (!GET_BIT(selectedChannel->SR, 6))
    ; // Wait transmission is not completed
}
...
u16 USART_u16Receive(u8 Channel) {
  ...
  while (!GET_BIT(selectedChannel->SR, 5))
 ...
 * @param  @param  Channel:	Number of the UART Channel to use.
 * 							Can be USART_CHANNEL_x where x is from 1
 * to 5
 ...
 * @retval u8 the 8/9 bits received in the data register
 */
Error_Status USART_enumDMAReceive(u8 UART_Channel, u8 DMA_Channel,
                                  volatile u32 *buffer, u32 Count) {
...
Error_Status USART_enumDMASend(u8 UART_Channel, u8 DMA_Channel,
                               volatile u8 *buffer, u32 Count) {
  u8 u8count = 0;
  UART_REG *selectedChannel = USART1;
