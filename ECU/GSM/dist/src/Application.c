...
/*				Range: checkupdate: if the respond with file update
 * version
 */
/* 					   VehicleNotFound: if the user name in the
 * update POST request not found in server database	*/
/* 					   Incorrect password: if the password in the
 * update POST request is incorrect					*/
/* Scope      : Public */
/********************************************************************************************************************/
...
/********************************************************************************************************************/
/* Description: It compare a certain file version on the server and the existing
 * version on a certain ECU			*/
/* Input      : u8 * pu8ServerResponse
 */
/*              Description: pointer to server response which contain a certain
 * file version						*/
/* 				u8 *  pu8CurrentECUVersion
 */
/* 				Description: pointer to a certain ECU software
 * version string which is must be written in the		*/
/* 							 following form
 * "vmajor.minor.patch" (Ex: "v2.1.5")
 */
/* Output     : Update_Status */
/*				Range: updateNotExist: if the update version and the
 * existing software version on a certain			*/
/* 					   				   ECU are the same
 */
/* 					   updateExist: if the update version is newer
 * than the existing software version on			*/
/* 									a certain ECU
 */
/* 					   updateRollbackExis: if the existing software
 * version on a certain ECU is newer than			*/
/*  									   the update version
 */
/* Scope      : Public */
/********************************************************************************************************************/
Update_Status updateVersioncheck(u8 *pu8ServerResponse,
                                 u8 *pu8CurrentECUVersion) {
  /*index [0] -> major, [1]-> minor, [2]->patch*/
  //	u16 currentECUversion[3];
  u16 serverFileversion[3];
  //	vidGetVersion(pu8CurrentECUVersion, &currentECUversion[0],
  //&currentECUversion[1], &currentECUversion[2]);
  vidGetVersion(pu8ServerResponse, &serverFileversion[0], &serverFileversion[1],
                &serverFileversion[2]);
  if (serverFileversion[0] == pu8CurrentECUVersion[0] &&
      serverFileversion[1] == pu8CurrentECUVersion[1] &&
      serverFileversion[2] == pu8CurrentECUVersion[2]) {
...
/******************************************************************************************************************/
/* Description: It compute HASH digest code according to SHA256	(Security
 * stack)							      */
...
/* 				u32 *MessageDigest
 */
/* 				Description: pointer to output parameter that will
...
/* Scope      : Public */
/******************************************************************************************************************/
s32 STM32_SHA256_HASH_DigestCompute(u8 *InputMessage, u32 InputMessageLength,
                                    u32 *MessageDigest,
                                    s32 *MessageDigestLength) {
  ...
  if (error_status == HASH_SUCCESS) {
    ...
    error_status =
        SHA256_Append(&P_pSHA256ctx, InputMessage, InputMessageLength);

    /* retrieve */
    if (error_status == HASH_SUCCESS) {
      error_status =
          SHA256_Finish(&P_pSHA256ctx, MessageDigest, MessageDigestLength);
    }
  }
...
/**************************************************************************************************************************/
/* Description: Compares Expected output which is extracted from server response
 * with the generated Hash digest code      */
/* note 	  : not designed for general buffer comparisons, its specific
 * design for handling the mismatching  			  */
/* 				between the received server format and the generated
 * hex Hash code
 */
/* Input      : u8 pBuffer[]
 */
/*              Description: Expected output which is extracted from server
 * response as ASCII string array				  */
/* 				Range	   : string of 64 ASCII characters
 */
/* 				u8* pBuffer1
 */
/* 				Description: pointer to the generated Hash digest
 * code
 */
/* 				Range 	   : 32 bytes of 32 double digit hex
 * number
 */
/* Output     : TestStatus
 */
...
TestStatus Buffercmp(u8 pBuffer[], u8 *pBuffer1) {
  ...
  for (int i = 1; i < 32; i += 2) {
    hex_ptr = (pBuffer[i - 1] <= '9') ? (pBuffer[i - 1] - '0') * 16
                                      : (pBuffer[i - 1] - 'a' + 10) * 16;
    hex_ptr +=
        (pBuffer[i] <= '9') ? (pBuffer[i] - '0') : (pBuffer[i] - 'a' + 10);
    if (hex_ptr != *(pBuffer1++)) {
      return FAILED;
    }
  }
...
/* Scope      : Private */
/****************************************************************************************/
static Error_Status enuFindString2(u8 *pu8Buffer, const u8 *pu8ExpectedString,
                                   u32 u32Size) {
...
/****************************************************************************************/
/* Description: It convert version ID from string form to major, minor and patch
 * values */
...
/*              Description: string of number needed to be converted to integer
 * value  	*/
/* 				Range: 	pointer to u8 array of characters
 */
/* Output     : void */
