    ...
    for (u8Counter = 0; u8Counter < 10; u8Counter++) {
      if (CANHANDLER_u8PendingFlag[u8Counter] == 0) {
        ...
        case CAN_u8REMOTEFRAME:
          ...
          CANHANDLER_strPendingMsg[u8Counter].id =
              u8MessageID; // initialise message to send
        ...
        case CAN_u8DATAFRAME:
          ...
          for (u8DataCounter = 0; u8DataCounter < u8DataLength;
               u8DataCounter++) {
            CANHANDLER_strPendingMsg[u8Counter].data[u8DataCounter] =
                pu8Data[u8DataCounter];
          }
    ...
    for (u8counter = 0; u8counter < 3; u8counter++) {
      if (CAN_RxMsg[u8counter].u8ActiveFlag == 1) {
        ...
        case CANHANDLER_u8ECUSWVERSION:
          for (u8DataCounter = 0; u8DataCounter < CAN_RxMsg[u8counter].len;
               u8DataCounter++) {
            CANHANDLER_au8ECUVersion[u8DataCounter] =
                CAN_RxMsg[u8counter].data[u8DataCounter];
          }
        ...
        case CANHANDLER_u8ECUDTCs:
          ...
          case CAN_u8DATAFRAME:
            for (u8DataCounter = 0; u8DataCounter < CAN_RxMsg[u8counter].len;
                 u8DataCounter++) {
              CANHANDLER_au8DTCs[u8DataCounter] =
                  CAN_RxMsg[u8counter].data[u8DataCounter];
            }
