...
Step GSMHANDLER_enuServerStep =
    GETSWVersion; /* Either GetSWVersion or GETHexFile or SendDiag*/
...
void GSMHANDLER_vidTask(void) {
  ...
  case SetURL:
    ...
    case GETSWVersion:
      ...
      if (CANHANDLER_u8UsedBank == 1) {
        ...
        // Application Bank 1
        vidSendCommand("AT+HTTPPARA=\"URL\",\"34.65.7.33/API/firmware/v/"
                       "5f2dbd21a594b91498537edb\"\r\n");
    ...
    case GETHexFile:
      if (CANHANDLER_u8UsedBank == 1) {
        ...
        // Application Bank 1
        vidSendCommand("AT+HTTPPARA=\"URL\",\"34.65.7.33/API/firmware/get/"
                       "5f2dbd21a594b91498537edb\"\r\n");
      } else if (CANHANDLER_u8UsedBank == 0) {
        // C14 on Bank 2 w/o feedback
    ...
    case SendDiag:
      /* Set URL of Diag */
      vidSendCommand("AT+HTTPPARA=\"URL\",\"34.65.7.33/API/vehicle/diagnostics/"
                     "11122\"\r\n");
  ...
  case CompareSWVersion:
    ...
    if (CANHANDLER_u8SWVersionReceived == 1) {
      ...
      if (u16Timeout >= 300) {
        u16Timeout = 0;
        CANHANDLER_vidSend(CANHANDLER_u8ECUSWVERSION, CAN_u8REMOTEFRAME, NULL,
                           0);
      }
  ...
  case AcceptUpdate:
    ...
    if (CANHANDLER_u8UpdateAcceptReceived == 1) {
      ...
      CANHANDLER_vidSend(CANHANDLER_u8UPDATEREQUESTID, CAN_u8REMOTEFRAME, NULL,
                         0);
  ...
  case CheckDecryption:
    ...
    status = STM32_SHA256_HASH_DigestCompute((u8 *)au8Data, u16ReceivedDataSize,
                                             (u8 *)MessageDigest,
                                             &MessageDigestLength);
    // if Hash generation succeed, Compare the received Hash code with the
    // generated Hash code
    if (status == HASH_SUCCESS) {
      ...
      /* Add application traintment in case of hash not success possible values
       * of status: HASH_ERR_BAD_PARAMETER, HASH_ERR_BAD_CONTEXT,
  ...
  case ReceiveFeedback:
    if (CANHANDLER_u8NextMsgRequest == 1) {
      ...
      if (GSMHANDLER_enuNextStep == GETHash) {
        ...
        u8UpdateProgress =
            (u8)((u32StartPoint * (u32)100) / u32ResponseDataSize);
        CANHANDLER_vidSend(CANHANDLER_u8UPDATEPROGRESS, CAN_u8DATAFRAME,
                           &u8UpdateProgress, 1);
        u32StartPoint += 264;
...
void vidSendVehicleName(void) {
  ...
  u8 u8DTCsCounter = 0;
  u8 u8DTCEndPoint = 0;
  static u8 au8VehicleNameDiag[150] =
      "{\"vehicleName\":\"fota user\",\"password\":\"123\",\"Diag\":\"DTCS: ";
  ...
  case IDLE:
    if (GSMHANDLER_enuServerStep == GETSWVersion) {
      USART_enumDMAReceive(GSM_u8USARTChannel, DMA_CHANNEL_5,
                           (u32 *)au8listenBuffer, 64);
      USART_enumDMASend(
          GSM_u8USARTChannel, DMA_CHANNEL_4,
          "{\"vehicleName\":\"fota user\",\"password\":\"123\"}\r\n", 0);
      GSM_enuListenFlag = WaitingForMessage;
      ...
      if (CANHANDLER_u8DTCsReceived == 1) {
        ...
        /* Add DTCs */
        u8Counter = 58;
        for (u8DTCsCounter = 0; CANHANDLER_au8DTCs[u8DTCsCounter] != 0;
             u8DTCsCounter++) {
          ...
          au8VehicleNameDiag[u8Counter] =
              ((CANHANDLER_au8DTCs[u8DTCsCounter] >> 4) & 0x0F) + '0';
          u8Counter++;
          au8VehicleNameDiag[u8Counter] =
              (CANHANDLER_au8DTCs[u8DTCsCounter] & 0x0F) + '0';
          u8Counter++;
          au8VehicleNameDiag[u8Counter] = ',';
          u8Counter++;
        }
        ...
        USART_enumDMASend(GSM_u8USARTChannel, DMA_CHANNEL_4, au8VehicleNameDiag,
                          0);
        GSM_enuListenFlag = WaitingForMessage;
      } else {
        /* Resend CAN Messages? */
      ...
      if (u8CheckBufferTermination(au8listenBuffer, 64, 4) == 1) {
        ...
        if (enuresponseStatus == OK) {
          ...
          if (enuresponseStatus == OK) {
            ...
            for (u8Counter = 27; au8listenBuffer[u8Counter] != '\r';
                 u8Counter++) {
              u32ReceivedDataSize = ((u32ReceivedDataSize)*10) +
                                    (au8listenBuffer[u8Counter] - '0');
            }
...
u16 u16GETData(u32 u32StartPoint, u16 u16DataLength, u8 *au8Data) {
  ...
  case IDLE:
    ...
    while (u8Counter2 < u8startasciistartpoint) {
      au8Cmd[u8Counter] =
          au8startpointascii[10 - u8startasciistartpoint + u8Counter2];
      u8Counter++;
      u8Counter2++;
    }
  ...
  case WaitingForMessage:
    ...
    if (GSMHANDLER_u8TransmissionCompleteFlag == 1) {
      ...
      if (enuresponseStatus == OK) {
        DMA_voidDisable(DMA_CHANNEL_5);
        for (u16Counter = 13; au8listenBuffer[u16Counter] != '\r';
             u16Counter++) {
          u16ReceivedDataSize =
              (u16ReceivedDataSize * 10) + (au8listenBuffer[u16Counter] - '0');
        }

        // The Received data is version info from server
        u16Counter = u16Counter + 2;
        u8DataStartPoint = u16Counter;
        for (; u16Counter < (u8DataStartPoint + u16ReceivedDataSize);
             u16Counter++) {
...
void vidCheckUpdate(u8 *au8Data) {
  ...
  if (serverStatus == checkupdate) {
    ...
    if (serverStatus == updateExist) {
      CANHANDLER_vidSend(CANHANDLER_u8UPDATEREQUESTGUI, CAN_u8REMOTEFRAME, NULL,
                         0);
...
void vidSendHexFile(u8 *au8Data, u16 u16ReceivedDataSize) {
  ...
  for (u8CharCount = 0; u8CharCount < 8;) {
    ...
    u8Counter++;
    if ((au8HexData[u8CharCount - 1] == '\r') ||
        (u8Counter == u16ReceivedDataSize)) {
      break;
    }
  }
  if (au8Data[u8Counter] == '\n') {
  ...
  CANHANDLER_vidSend(CANHANDLER_u8HEXFILEID, CAN_u8DATAFRAME, au8HexData,
                     u8CharCount);
...
/* Scope      : Private */
/****************************************************************************************/
void vidDMAIRQ(void) { GSMHANDLER_u8TransmissionCompleteFlag = 1; }
