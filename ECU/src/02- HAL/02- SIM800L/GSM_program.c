...
u16 GSM_u16GETData(u32 u32StartPoint, u16 u16DataLength, u8* au8Data)
{
	...
	while ( !u8CheckBufferTermination( au8Response, u16ResponseSize, 4 ) );
	DMA_voidDisable( DMA_CHANNEL_5 );
	...
	if (enuFindString( (au8Response+u16Counter), "OK", 1100-u16Counter ) == NOK)
	{
		return 0;
	}
...
/* Scope      : Public                                                                 	*/
/****************************************************************************************/
void GSM_enuPOSTRequestInit(const u8* pu8URL, const u8* postRequestData, u8* responseData)
{
	volatile u8 au8ResponseBuffer[64] = {0};		/* \r\nOK\r\n or \r\nERROR\r\n */
	Error_Status enuErrorCheck = OK;
	u32 u32count=0;
	...
	USART_enumDMAReceive( GSM_u8USARTChannel, DMA_CHANNEL_5, (u32*) au8ResponseBuffer, 64 );
	USART_voidSendString(GSM_u8USARTChannel, "AT+HTTPDATA=100,2000");
	USART_voidSendString(GSM_u8USARTChannel, "\r\n");
...
//	USART_voidSendString(GSM_u8USARTChannel, "\r\n");
	...
	if ( au8ResponseBuffer[17] == '2' && au8ResponseBuffer[18] == '0' && au8ResponseBuffer[19] == '0' )
	{
		//succesful HTTP POST request  and response
//		for (u32Counter = 21; au8ResponseBuffer[u32Counter] != '\r'; u32Counter++)
//		{
//			u32ReturnValue = (u32ReturnValue * 10) + (au8ResponseBuffer[u32Counter] - '0');
//		}
...
//	u8 u8count;
//	u8 u8count2;
//	for(u8count=0; u8count<100; u8count++)
//	{
//		if(au8ResponseBuffer[u8count]=='\r' && au8ResponseBuffer[u8count+1]=='\n')
//		{
//			u8repeat++;
//			u8count++;
//		}
...
//	u8count++;
//
//	for(u8count2=0; u8count2<100; u8count2++)
//	{
//		responseData[u8count2]=au8ResponseBuffer[u8count];
//		u8count++;
//		if(au8ResponseBuffer[u8count]=='\r' && au8ResponseBuffer[u8count+1]=='\n')
//		{
//			responseData[u8count2+1]='\0';
////			responseData[u8count2+2]=au8ResponseBuffer[u8count+1];
//			break;
//		}
//	}
}
...
//void GSM_enuPOSTRequestInit(const u8* pu8URL, const u8* postRequestData)
